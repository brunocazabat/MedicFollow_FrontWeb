...
export default {
  ...
  methods: {
    ...
    updateorder() {
      let result = this.sugsList.findIndex(
        (o) => o.id == document.getElementById("orderId").value
      );
      this.sugsList[result].title =
        document.getElementById("tasksTitle").value;
      this.sugsList[result].client =
        document.getElementById("clientName").value;
      this.sugsList[result].create = document.getElementById("cdate").value;
      this.sugsList[result].status =
    ...
    deletedata(event) {
      Swal.fire({
        ...
        if (result.value) {
          this.sugsList.splice(this.sugsList.indexOf(event), 1);
    ...
    deleteMultiple() {
      ...
      if (typeof ids_array !== "undefined" && ids_array.length > 0) {
        if (confirm("Are you sure you want to delete this?")) {
          let cusList = this.sugsList;
          ids_array.forEach(function (id) {
            cusList = cusList.filter(function (orders) {
              return orders.id != id;
            });
          });
          this.sugsList = cusList;
    ...
    addorder() {
      let id = "#MDC" + this.sugsList.length + 1;
