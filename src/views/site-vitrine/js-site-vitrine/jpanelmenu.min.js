...
! function(a) {
    a.jPanelMenu = function(b) {
        ...
        var c = {
            ...
            timeouts: {},
            clearTimeouts: function() { clearTimeout(c.timeouts.open), clearTimeout(c.timeouts.afterOpen), clearTimeout(c.timeouts.afterClose) },
            ...
            setCSSPrefix: function() { c.settings.cssPrefix = c.getCSSPrefix() },
            setjPanelMenuStyles: function() {
                var b = "background:#fff",
                    d = a("html").css("background-color"),
                    e = a("body").css("background-color"),
                    f = function(b) { var c = []; return a.each(["background-color", "background-image", "background-position", "background-repeat", "background-attachment", "background-size", "background-clip"], function(a, d) { "" !== b.css(d) && c.push(d + ":" + b.css(d)) }), c.join(";") };
            ...
            getMenuState: function() { return a(c.options.panel).attr("data-menu-position") },
            menuIsOpen: function() { return "open" == c.getMenuState() },
            setMenuStyle: function(b) { a(c.menu).css(b) },
            setPanelStyle: function(b) { a(c.panel).css(b) },
            showMenu: function() { c.setMenuStyle({ display: "block" }), c.setMenuStyle({ "z-index": "1" }) },
            hideMenu: function() { c.setMenuStyle({ "z-index": "-1" }), c.setMenuStyle({ display: "none" }) },
            ...
            disableTransitions: function() { a("#jPanelMenu-style-transitions").remove() },
            ...
            getCSSPrefix: function() { var a = c.getVendorPrefix(); return "" != a ? "-" + a.toLowerCase() + "-" : "" },
            openMenu: function(b) {
                ...
                if (d.transitions || d.none) {
                    ...
                    c.setPanelStyle(e), c.timeouts.afterOpen = setTimeout(function() { c.options.after(), c.options.afterOpen(), c.initiateContentClickListeners() }, c.options.openDuration)
                } else {
                    var f = c.getJSEasingFunction(c.options.openEasing),
                        g = {};
                    g[c.options.direction] = c.options.openPosition, a(c.panel).stop().animate(g, c.options.openDuration, f, function() { c.options.after(), c.options.afterOpen(), c.initiateContentClickListeners() })
                }
            },
            closeMenu: function(b) {
                ...
                if (d.transitions || d.none) {
                    ...
                    c.setPanelStyle(e), c.timeouts.afterClose = setTimeout(function() { c.disableTransitions(), c.hideMenu(), c.options.after(), c.options.afterClose(), c.destroyContentClickListeners() }, c.options.closeDuration)
                } else {
                    var f = c.getJSEasingFunction(c.options.closeEasing),
                        g = {};
                    g[c.options.direction] = 0 + c.settings.positionUnits, a(c.panel).stop().animate(g, c.options.closeDuration, f, function() { c.hideMenu(), c.options.after(), c.options.afterClose(), c.destroyContentClickListeners() })
                }
            },
            triggerMenu: function(a) { c.menuIsOpen() ? c.closeMenu(a) : c.openMenu(a) },
            initiateClickListeners: function() { a(document).on("click touchend", c.options.trigger, function(a) { c.triggerMenu(c.options.animated), a.preventDefault() }) },
            destroyClickListeners: function() { a(document).off("click touchend", c.options.trigger, null) },
            initiateContentClickListeners: function() { return !!c.options.closeOnContentClick && void a(document).on("click touchend", c.panel, function(a) { c.menuIsOpen() && c.closeMenu(c.options.animated), a.preventDefault() }) },
            destroyContentClickListeners: function() { return !!c.options.closeOnContentClick && void a(document).off("click touchend", c.panel, null) },
            initiateKeyboardListeners: function() {
                var b = ["input", "textarea", "select"];
                a(document).on("keydown", function(d) {
                    var e = a(d.target),
                        f = !1;
                    if (a.each(b, function() { e.is(this.toString()) && (f = !0) }), f) return !0;
            ...
            destroyKeyboardListeners: function() { a(document).off("keydown", null) },
